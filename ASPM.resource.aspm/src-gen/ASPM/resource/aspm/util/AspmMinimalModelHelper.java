 ...
 */
public class AspmMinimalModelHelper {
	
	private final static ASPM.resource.aspm.util.AspmEClassUtil eClassUtil = new ASPM.resource.aspm.util.AspmEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + ASPM.resource.aspm.util.AspmStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = ASPM.resource.aspm.util.AspmListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + ASPM.resource.aspm.util.AspmStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = ASPM.resource.aspm.util.AspmListUtil.castListUnchecked(value);
