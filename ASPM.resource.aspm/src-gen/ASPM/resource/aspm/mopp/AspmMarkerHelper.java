 ...
 * contains <code>ASPM.resource.aspm.IAspmTextDiagnostic</code>s it uses the more
 * precise information of this extended diagnostic type.
 */
public class AspmMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = ASPM.resource.aspm.mopp.AspmPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<ASPM.resource.aspm.IAspmCommand<Object>> commands = new java.util.ArrayList<ASPM.resource.aspm.IAspmCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(ASPM.resource.aspm.IAspmCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<ASPM.resource.aspm.IAspmCommand<Object>> commandsToProcess = new java.util.ArrayList<ASPM.resource.aspm.IAspmCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (ASPM.resource.aspm.IAspmCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, ASPM.resource.aspm.IAspmTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final ASPM.resource.aspm.IAspmTextDiagnostic diagnostic) {
		final ASPM.resource.aspm.IAspmProblem problem = diagnostic.getProblem();
		ASPM.resource.aspm.AspmEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new ASPM.resource.aspm.IAspmCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == ASPM.resource.aspm.AspmEProblemSeverity.ERROR) {
					...
					ASPM.resource.aspm.IAspmTextDiagnostic textDiagnostic = (ASPM.resource.aspm.IAspmTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof ASPM.resource.aspm.mopp.AspmResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((ASPM.resource.aspm.mopp.AspmResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<ASPM.resource.aspm.IAspmQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, ASPM.resource.aspm.util.AspmStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (ASPM.resource.aspm.AspmEProblemType nextType : ASPM.resource.aspm.AspmEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, ASPM.resource.aspm.AspmEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(ASPM.resource.aspm.AspmEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(ASPM.resource.aspm.AspmEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new ASPM.resource.aspm.util.AspmRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new ASPM.resource.aspm.IAspmCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					ASPM.resource.aspm.mopp.AspmPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
