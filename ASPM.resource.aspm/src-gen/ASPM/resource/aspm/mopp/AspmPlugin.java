 ...
 */
public class AspmPlugin extends org.eclipse.core.runtime.Plugin {
	
	public static final String PLUGIN_ID = "ASPM.resource.aspm";
	/**
	...
	private static AspmPlugin plugin;
	...
	public static AspmPlugin getDefault() {
		return plugin;
	}
	...
	protected static org.eclipse.core.runtime.IStatus log(int type, String message, Throwable throwable) {
		...
		if (throwable != null) {
			status = new org.eclipse.core.runtime.Status(type, AspmPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new org.eclipse.core.runtime.Status(type, AspmPlugin.PLUGIN_ID, message);
		}
		final AspmPlugin pluginInstance = AspmPlugin.getDefault();
