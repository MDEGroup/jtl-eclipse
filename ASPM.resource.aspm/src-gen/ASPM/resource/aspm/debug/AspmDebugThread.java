...
public class AspmDebugThread extends ASPM.resource.aspm.debug.AspmDebugElement implements org.eclipse.debug.core.model.IThread, ASPM.resource.aspm.debug.IAspmDebugEventListener {
	
	private boolean suspended = false;
	private ASPM.resource.aspm.debug.AspmDebugTarget debugTarget;
	
	public AspmDebugThread(ASPM.resource.aspm.debug.AspmDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			ASPM.resource.aspm.debug.AspmDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = ASPM.resource.aspm.util.AspmStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new ASPM.resource.aspm.debug.AspmStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public ASPM.resource.aspm.debug.AspmDebugTarget getTarget() {
		return debugTarget;
	}
