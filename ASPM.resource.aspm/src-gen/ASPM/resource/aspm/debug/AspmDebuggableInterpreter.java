 ...
 */
public class AspmDebuggableInterpreter<ResultType, ContextType> extends ASPM.resource.aspm.debug.AbstractAspmDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private ASPM.resource.aspm.util.AbstractAspmInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private ASPM.resource.aspm.IAspmCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public AspmDebuggableInterpreter(ASPM.resource.aspm.util.AbstractAspmInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new ASPM.resource.aspm.IAspmInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = ASPM.resource.aspm.util.AspmStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public ASPM.resource.aspm.util.AbstractAspmInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		ASPM.resource.aspm.IAspmLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private ASPM.resource.aspm.IAspmLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		ASPM.resource.aspm.IAspmCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = ASPM.resource.aspm.util.AspmEObjectUtil.getDepth(current);
		stopCondition = new ASPM.resource.aspm.IAspmCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = ASPM.resource.aspm.util.AspmEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = ASPM.resource.aspm.util.AspmEObjectUtil.getDepth(current) - 1;
		stopCondition = new ASPM.resource.aspm.IAspmCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
