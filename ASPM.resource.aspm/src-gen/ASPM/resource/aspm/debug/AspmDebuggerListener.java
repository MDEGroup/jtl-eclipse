 ...
 * code).
 */
public class AspmDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private ASPM.resource.aspm.debug.AbstractAspmDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, ASPM.resource.aspm.util.AspmPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, ASPM.resource.aspm.util.AspmPair<String, Object>>();
	...
	public AspmDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		ASPM.resource.aspm.debug.AspmDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = ASPM.resource.aspm.util.AspmStringUtil.encode('#', stack);
				ASPM.resource.aspm.debug.AspmDebugMessage message = new ASPM.resource.aspm.debug.AspmDebugMessage(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				ASPM.resource.aspm.debug.AspmDebugMessage message = new ASPM.resource.aspm.debug.AspmDebugMessage(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.GET_VARIABLES)) {
				...
				ASPM.resource.aspm.debug.AspmDebugMessage message = new ASPM.resource.aspm.debug.AspmDebugMessage(ASPM.resource.aspm.debug.EAspmDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return ASPM.resource.aspm.util.AspmStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		ASPM.resource.aspm.util.AspmPair<String, Object> pair = new ASPM.resource.aspm.util.AspmPair<String, Object>(name, value);
	...
	public ASPM.resource.aspm.debug.AbstractAspmDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(ASPM.resource.aspm.debug.AbstractAspmDebuggable debuggable) {
