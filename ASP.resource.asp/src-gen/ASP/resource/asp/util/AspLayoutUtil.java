...
public class AspLayoutUtil {
	...
	public void transferLayoutInformationToModel(org.eclipse.emf.ecore.EObject element) {
		...
		for (java.util.Iterator<ASP.resource.asp.mopp.AspLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			ASP.resource.asp.mopp.AspLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(org.eclipse.emf.ecore.EObject element) {
		...
		if (layoutReference != null) {
			...
			for (java.util.Iterator<org.eclipse.emf.ecore.EObject> i = list.iterator(); i.hasNext(); ) {
				...
				ASP.resource.asp.mopp.AspLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public org.eclipse.emf.ecore.EObject createLayoutInformationModelElement(ASP.resource.asp.mopp.AspLayoutInformation layoutInformation, org.eclipse.emf.ecore.EPackage layoutPackage) {
		...
		ASP.resource.asp.grammar.AspSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), ASP.resource.asp.grammar.AspGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public ASP.resource.asp.mopp.AspLayoutInformation createLayoutInformation(org.eclipse.emf.ecore.EObject layoutInformationModelElement) {
		Object object = null;
		...
		ASP.resource.asp.grammar.AspSyntaxElement syntaxElement = ASP.resource.asp.grammar.AspGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof ASP.resource.asp.grammar.AspPlaceholder) {
			ASP.resource.asp.grammar.AspPlaceholder placeholder = (ASP.resource.asp.grammar.AspPlaceholder) syntaxElement;
			ASP.resource.asp.IAspTokenResolver tokenResolver = new ASP.resource.asp.mopp.AspTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			ASP.resource.asp.IAspTokenResolveResult result = new ASP.resource.asp.mopp.AspTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new ASP.resource.asp.mopp.AspLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public ASP.resource.asp.mopp.AspLayoutInformationAdapter getLayoutInformationAdapter(org.eclipse.emf.ecore.EObject element) {
