...
public class AspDebugTarget extends ASP.resource.asp.debug.AspDebugElement implements org.eclipse.debug.core.model.IDebugTarget, ASP.resource.asp.debug.IAspDebugEventListener {
	
	/**
	 * associated system process
	 */
	private ASP.resource.asp.debug.AspDebugProcess process;
	...
	private ASP.resource.asp.debug.AspDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<ASP.resource.asp.debug.IAspDebugEventListener> eventListeners = new java.util.ArrayList<ASP.resource.asp.debug.IAspDebugEventListener>();
	
	private ASP.resource.asp.debug.AspDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new ASP.resource.asp.mopp.AspMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				ASP.resource.asp.debug.AspDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(ASP.resource.asp.debug.AspDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((ASP.resource.asp.debug.IAspDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public AspDebugTarget(ASP.resource.asp.debug.AspDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new ASP.resource.asp.debug.AspDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new ASP.resource.asp.debug.AspDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(ASP.resource.asp.debug.IAspDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(ASP.resource.asp.debug.IAspDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new ASP.resource.asp.mopp.AspMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					ASP.resource.asp.debug.AspLineBreakpoint lineBreakpoint = (ASP.resource.asp.debug.AspLineBreakpoint) breakpoint;
	...
	public void handleMessage(ASP.resource.asp.debug.AspDebugMessage message) {
		try {
			if (message.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.RESUMED)) {
	...
	public ASP.resource.asp.debug.AspDebugProxy getDebugProxy() {
