 ...
 * code).
 */
public class AspDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private ASP.resource.asp.debug.AbstractAspDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, ASP.resource.asp.util.AspPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, ASP.resource.asp.util.AspPair<String, Object>>();
	...
	public AspDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		ASP.resource.asp.debug.AspDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = ASP.resource.asp.util.AspStringUtil.encode('#', stack);
				ASP.resource.asp.debug.AspDebugMessage message = new ASP.resource.asp.debug.AspDebugMessage(ASP.resource.asp.debug.EAspDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				ASP.resource.asp.debug.AspDebugMessage message = new ASP.resource.asp.debug.AspDebugMessage(ASP.resource.asp.debug.EAspDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(ASP.resource.asp.debug.EAspDebugMessageTypes.GET_VARIABLES)) {
				...
				ASP.resource.asp.debug.AspDebugMessage message = new ASP.resource.asp.debug.AspDebugMessage(ASP.resource.asp.debug.EAspDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return ASP.resource.asp.util.AspStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		ASP.resource.asp.util.AspPair<String, Object> pair = new ASP.resource.asp.util.AspPair<String, Object>(name, value);
	...
	public ASP.resource.asp.debug.AbstractAspDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(ASP.resource.asp.debug.AbstractAspDebuggable debuggable) {
