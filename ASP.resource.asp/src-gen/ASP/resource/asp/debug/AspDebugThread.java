...
public class AspDebugThread extends ASP.resource.asp.debug.AspDebugElement implements org.eclipse.debug.core.model.IThread, ASP.resource.asp.debug.IAspDebugEventListener {
	
	private boolean suspended = false;
	private ASP.resource.asp.debug.AspDebugTarget debugTarget;
	
	public AspDebugThread(ASP.resource.asp.debug.AspDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			ASP.resource.asp.debug.AspDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = ASP.resource.asp.util.AspStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new ASP.resource.asp.debug.AspStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public ASP.resource.asp.debug.AspDebugTarget getTarget() {
		return debugTarget;
	}
