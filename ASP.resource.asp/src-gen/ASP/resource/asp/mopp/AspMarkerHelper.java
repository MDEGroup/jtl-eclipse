 ...
 * contains <code>ASP.resource.asp.IAspTextDiagnostic</code>s it uses the more
 * precise information of this extended diagnostic type.
 */
public class AspMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = ASP.resource.asp.mopp.AspPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<ASP.resource.asp.IAspCommand<Object>> commands = new java.util.ArrayList<ASP.resource.asp.IAspCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(ASP.resource.asp.IAspCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<ASP.resource.asp.IAspCommand<Object>> commandsToProcess = new java.util.ArrayList<ASP.resource.asp.IAspCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (ASP.resource.asp.IAspCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, ASP.resource.asp.IAspTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final ASP.resource.asp.IAspTextDiagnostic diagnostic) {
		final ASP.resource.asp.IAspProblem problem = diagnostic.getProblem();
		ASP.resource.asp.AspEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new ASP.resource.asp.IAspCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == ASP.resource.asp.AspEProblemSeverity.ERROR) {
					...
					ASP.resource.asp.IAspTextDiagnostic textDiagnostic = (ASP.resource.asp.IAspTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof ASP.resource.asp.mopp.AspResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((ASP.resource.asp.mopp.AspResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<ASP.resource.asp.IAspQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, ASP.resource.asp.util.AspStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (ASP.resource.asp.AspEProblemType nextType : ASP.resource.asp.AspEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, ASP.resource.asp.AspEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(ASP.resource.asp.AspEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(ASP.resource.asp.AspEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new ASP.resource.asp.util.AspRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new ASP.resource.asp.IAspCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					ASP.resource.asp.mopp.AspPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
