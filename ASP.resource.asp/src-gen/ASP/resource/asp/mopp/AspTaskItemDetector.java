...
/**
 * The AspTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * AspTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class AspTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<ASP.resource.asp.mopp.AspTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<ASP.resource.asp.mopp.AspTaskItem> foundItems = new java.util.ArrayList<ASP.resource.asp.mopp.AspTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new ASP.resource.asp.mopp.AspTaskItem(keyword, message, localLine, offset, end));
