 ...
 */
public class AspmmOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "ASPMM.resource.aspmm.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "ASPMM.resource.aspmm.ui.occurences.declaration";
	...
	private ASPMM.resource.aspmm.ui.AspmmTokenScanner tokenScanner;
	 ...
	 */
	public AspmmOccurrence(ASPMM.resource.aspmm.IAspmmTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, ASPMM.resource.aspmm.ui.AspmmTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new ASPMM.resource.aspmm.mopp.AspmmMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(ASPMM.resource.aspmm.ui.AspmmBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, ASPMM.resource.aspmm.ui.AspmmPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, ASPMM.resource.aspmm.ui.AspmmPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, ASPMM.resource.aspmm.ui.AspmmPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == ASPMM.resource.aspmm.ui.AspmmPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(ASPMM.resource.aspmm.ui.AspmmOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(ASPMM.resource.aspmm.ui.AspmmOccurrence.DECLARATION_ANNOTATION_ID);
	}
