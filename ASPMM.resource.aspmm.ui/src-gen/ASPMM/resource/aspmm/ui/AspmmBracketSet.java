...
public class AspmmBracketSet {
	...
	private java.util.ArrayList<ASPMM.resource.aspmm.IAspmmBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements ASPMM.resource.aspmm.IAspmmBracketPair {
	 ...
	 */
	private class ClosingListener implements ASPMM.resource.aspmm.ui.IAspmmBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				ASPMM.resource.aspmm.IAspmmBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public AspmmBracketSet(ASPMM.resource.aspmm.ui.AspmmEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new ASPMM.resource.aspmm.mopp.AspmmMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<ASPMM.resource.aspmm.IAspmmBracketPair>();
	 ...
	 */
	public ASPMM.resource.aspmm.IAspmmBracketPair getBracketPair(String opening, String closing) {
		for (ASPMM.resource.aspmm.IAspmmBracketPair bracketPair : bracketPairs) {
	...
	public ASPMM.resource.aspmm.IAspmmBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(ASPMM.resource.aspmm.IAspmmBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + ASPMM.resource.aspmm.ui.AspmmPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public ASPMM.resource.aspmm.IAspmmBracketPair remove(String opening, String closing) {
		for (ASPMM.resource.aspmm.IAspmmBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<ASPMM.resource.aspmm.IAspmmBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, ASPMM.resource.aspmm.ui.AspmmPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, ASPMM.resource.aspmm.ui.AspmmPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
