 ...
 */
public class AspmmHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static ASPMM.resource.aspmm.ui.AspmmPositionHelper positionHelper = new ASPMM.resource.aspmm.ui.AspmmPositionHelper();
	...
	private ASPMM.resource.aspmm.ui.AspmmTokenScanner scanner;
	...
	private ASPMM.resource.aspmm.ui.AspmmEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private ASPMM.resource.aspmm.ui.AspmmOccurrence occurrence;
	private ASPMM.resource.aspmm.ui.AspmmBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, ASPMM.resource.aspmm.IAspmmBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public AspmmHighlighting(ASPMM.resource.aspmm.IAspmmTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, ASPMM.resource.aspmm.ui.AspmmColorManager colorManager, ASPMM.resource.aspmm.ui.AspmmEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = ASPMM.resource.aspmm.ui.AspmmUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new ASPMM.resource.aspmm.ui.AspmmTokenScanner(textResource, colorManager);
		occurrence = new ASPMM.resource.aspmm.ui.AspmmOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new ASPMM.resource.aspmm.ui.AspmmBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(ASPMM.resource.aspmm.ui.AspmmPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, ASPMM.resource.aspmm.ui.AspmmPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(ASPMM.resource.aspmm.ui.AspmmEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, ASPMM.resource.aspmm.ui.AspmmPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, ASPMM.resource.aspmm.ui.AspmmPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(ASPMM.resource.aspmm.ui.AspmmPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new ASPMM.resource.aspmm.ui.AspmmEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
