...
/**
 * A text editor for 'aspmm' models.
 * <p>
 * This editor has id <code>ASPMM.resource.aspmm.ui.AspmmEditor</code>
 * The editor's context menu has id
 * <code>ASPMM.resource.aspmm.EditorContext</code>.
 * The editor's ruler context menu has id
 * <code>ASPMM.resource.aspmm.EditorRuler</code>.
 * </p>
 */
public class AspmmEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, ASPMM.resource.aspmm.IAspmmResourceProvider, ASPMM.resource.aspmm.ui.IAspmmBracketHandlerProvider, ASPMM.resource.aspmm.ui.IAspmmAnnotationModelProvider {
	
	private ASPMM.resource.aspmm.ui.AspmmHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private ASPMM.resource.aspmm.ui.AspmmCodeFoldingManager codeFoldingManager;
	private ASPMM.resource.aspmm.ui.AspmmBackgroundParsingStrategy bgParsingStrategy = new ASPMM.resource.aspmm.ui.AspmmBackgroundParsingStrategy();
	private java.util.Collection<ASPMM.resource.aspmm.IAspmmBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<ASPMM.resource.aspmm.IAspmmBackgroundParsingListener>();
	private ASPMM.resource.aspmm.ui.AspmmColorManager colorManager = new ASPMM.resource.aspmm.ui.AspmmColorManager();
	private ASPMM.resource.aspmm.ui.AspmmOutlinePage outlinePage;
	...
	private ASPMM.resource.aspmm.ui.AspmmPropertySheetPage propertySheetPage;
	...
	private ASPMM.resource.aspmm.ui.IAspmmBracketHandler bracketHandler;
	...
	public AspmmEditor() {
		super();
		setSourceViewerConfiguration(new ASPMM.resource.aspmm.ui.AspmmSourceViewerConfiguration(this, this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), AspmmEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								ASPMM.resource.aspmm.IAspmmTextResource currentResource = getResource();
				...
				ASPMM.resource.aspmm.ui.AspmmUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("ASPMM.resource.aspmm.EditorContext");
		setRulerContextMenuId("ASPMM.resource.aspmm.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new ASPMM.resource.aspmm.ui.AspmmHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new ASPMM.resource.aspmm.ui.AspmmCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		ASPMM.resource.aspmm.mopp.AspmmNature.activate(inputFile.getProject());
		...
		ASPMM.resource.aspmm.IAspmmTextResource loadedResource = (ASPMM.resource.aspmm.IAspmmTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				ASPMM.resource.aspmm.IAspmmTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof ASPMM.resource.aspmm.IAspmmTextResource) {
					setResource((ASPMM.resource.aspmm.IAspmmTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					ASPMM.resource.aspmm.ui.AspmmUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the AspmmEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				ASPMM.resource.aspmm.ui.AspmmUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(ASPMM.resource.aspmm.IAspmmTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new ASPMM.resource.aspmm.ui.AspmmOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new ASPMM.resource.aspmm.ui.AspmmPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			ASPMM.resource.aspmm.IAspmmTextResource textResource = (ASPMM.resource.aspmm.IAspmmTextResource) element.eResource();
			...
			ASPMM.resource.aspmm.IAspmmTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				ASPMM.resource.aspmm.IAspmmTextToken token = lexer.getNextToken();
			...
			ASPMM.resource.aspmm.ui.AspmmUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!AspmmEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(ASPMM.resource.aspmm.IAspmmBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (ASPMM.resource.aspmm.IAspmmBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public ASPMM.resource.aspmm.ui.IAspmmBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(ASPMM.resource.aspmm.ui.IAspmmBracketHandler bracketHandler) {
	...
	private boolean setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
		if (selection instanceof org.eclipse.jface.viewers.IStructuredSelection) {
			...
			if (object instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (!(resource instanceof ASPMM.resource.aspmm.IAspmmTextResource)) {
					return false;
				}
				ASPMM.resource.aspmm.IAspmmTextResource textResource = (ASPMM.resource.aspmm.IAspmmTextResource) resource;
				ASPMM.resource.aspmm.IAspmmLocationMap locationMap = textResource.getLocationMap();
				int destination = locationMap.getCharStart(element);
				if (destination < 0) {
					destination = 0;
