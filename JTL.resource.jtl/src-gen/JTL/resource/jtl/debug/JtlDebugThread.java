...
public class JtlDebugThread extends JTL.resource.jtl.debug.JtlDebugElement implements org.eclipse.debug.core.model.IThread, JTL.resource.jtl.debug.IJtlDebugEventListener {
	
	private boolean suspended = false;
	private JTL.resource.jtl.debug.JtlDebugTarget debugTarget;
	
	public JtlDebugThread(JTL.resource.jtl.debug.JtlDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			JTL.resource.jtl.debug.JtlDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = JTL.resource.jtl.util.JtlStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new JTL.resource.jtl.debug.JtlStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public JTL.resource.jtl.debug.JtlDebugTarget getTarget() {
		return debugTarget;
	}
