 ...
 */
public class JtlDebuggableInterpreter<ResultType, ContextType> extends JTL.resource.jtl.debug.AbstractJtlDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private JTL.resource.jtl.util.AbstractJtlInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private JTL.resource.jtl.IJtlCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public JtlDebuggableInterpreter(JTL.resource.jtl.util.AbstractJtlInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new JTL.resource.jtl.IJtlInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = JTL.resource.jtl.util.JtlStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public JTL.resource.jtl.util.AbstractJtlInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		JTL.resource.jtl.IJtlLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private JTL.resource.jtl.IJtlLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		JTL.resource.jtl.IJtlCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(JTL.resource.jtl.debug.EJtlDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = JTL.resource.jtl.util.JtlEObjectUtil.getDepth(current);
		stopCondition = new JTL.resource.jtl.IJtlCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = JTL.resource.jtl.util.JtlEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = JTL.resource.jtl.util.JtlEObjectUtil.getDepth(current) - 1;
		stopCondition = new JTL.resource.jtl.IJtlCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
