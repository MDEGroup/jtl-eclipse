 ...
 * code).
 */
public class JtlDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private JTL.resource.jtl.debug.AbstractJtlDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, JTL.resource.jtl.util.JtlPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, JTL.resource.jtl.util.JtlPair<String, Object>>();
	...
	public JtlDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		JTL.resource.jtl.debug.JtlDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(JTL.resource.jtl.debug.EJtlDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(JTL.resource.jtl.debug.EJtlDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(JTL.resource.jtl.debug.EJtlDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(JTL.resource.jtl.debug.EJtlDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(JTL.resource.jtl.debug.EJtlDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(JTL.resource.jtl.debug.EJtlDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(JTL.resource.jtl.debug.EJtlDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(JTL.resource.jtl.debug.EJtlDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = JTL.resource.jtl.util.JtlStringUtil.encode('#', stack);
				JTL.resource.jtl.debug.JtlDebugMessage message = new JTL.resource.jtl.debug.JtlDebugMessage(JTL.resource.jtl.debug.EJtlDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(JTL.resource.jtl.debug.EJtlDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				JTL.resource.jtl.debug.JtlDebugMessage message = new JTL.resource.jtl.debug.JtlDebugMessage(JTL.resource.jtl.debug.EJtlDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(JTL.resource.jtl.debug.EJtlDebugMessageTypes.GET_VARIABLES)) {
				...
				JTL.resource.jtl.debug.JtlDebugMessage message = new JTL.resource.jtl.debug.JtlDebugMessage(JTL.resource.jtl.debug.EJtlDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return JTL.resource.jtl.util.JtlStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		JTL.resource.jtl.util.JtlPair<String, Object> pair = new JTL.resource.jtl.util.JtlPair<String, Object>(name, value);
	...
	public JTL.resource.jtl.debug.AbstractJtlDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(JTL.resource.jtl.debug.AbstractJtlDebuggable debuggable) {
