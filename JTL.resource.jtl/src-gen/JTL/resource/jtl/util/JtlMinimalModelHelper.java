 ...
 */
public class JtlMinimalModelHelper {
	
	private final static JTL.resource.jtl.util.JtlEClassUtil eClassUtil = new JTL.resource.jtl.util.JtlEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + JTL.resource.jtl.util.JtlStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = JTL.resource.jtl.util.JtlListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + JTL.resource.jtl.util.JtlStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = JTL.resource.jtl.util.JtlListUtil.castListUnchecked(value);
