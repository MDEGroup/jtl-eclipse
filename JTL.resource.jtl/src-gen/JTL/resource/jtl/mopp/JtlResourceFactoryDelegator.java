...
public class JtlResourceFactoryDelegator implements org.eclipse.emf.ecore.resource.Resource.Factory {
	
	protected java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories = null;
	
	public JtlResourceFactoryDelegator() {
		init();
	}
	...
	protected void init() {
		...
		if (new JTL.resource.jtl.util.JtlRuntimeUtil().isEclipsePlatformAvailable()) {
			new JTL.resource.jtl.util.JtlEclipseProxy().getResourceFactoryExtensions(factories);
		}
		if (factories.get("") == null) {
			factories.put("", new JTL.resource.jtl.mopp.JtlResourceFactory());
		}
	}
