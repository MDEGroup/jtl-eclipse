 ...
 * contains <code>JTL.resource.jtl.IJtlTextDiagnostic</code>s it uses the more
 * precise information of this extended diagnostic type.
 */
public class JtlMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = JTL.resource.jtl.mopp.JtlPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<JTL.resource.jtl.IJtlCommand<Object>> commands = new java.util.ArrayList<JTL.resource.jtl.IJtlCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(JTL.resource.jtl.IJtlCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<JTL.resource.jtl.IJtlCommand<Object>> commandsToProcess = new java.util.ArrayList<JTL.resource.jtl.IJtlCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (JTL.resource.jtl.IJtlCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, JTL.resource.jtl.IJtlTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final JTL.resource.jtl.IJtlTextDiagnostic diagnostic) {
		final JTL.resource.jtl.IJtlProblem problem = diagnostic.getProblem();
		JTL.resource.jtl.JtlEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new JTL.resource.jtl.IJtlCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == JTL.resource.jtl.JtlEProblemSeverity.ERROR) {
					...
					JTL.resource.jtl.IJtlTextDiagnostic textDiagnostic = (JTL.resource.jtl.IJtlTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof JTL.resource.jtl.mopp.JtlResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((JTL.resource.jtl.mopp.JtlResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<JTL.resource.jtl.IJtlQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, JTL.resource.jtl.util.JtlStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (JTL.resource.jtl.JtlEProblemType nextType : JTL.resource.jtl.JtlEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, JTL.resource.jtl.JtlEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(JTL.resource.jtl.JtlEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(JTL.resource.jtl.JtlEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new JTL.resource.jtl.util.JtlRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new JTL.resource.jtl.IJtlCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					JTL.resource.jtl.mopp.JtlPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
