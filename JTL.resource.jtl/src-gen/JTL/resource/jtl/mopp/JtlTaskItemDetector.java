...
/**
 * The JtlTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * JtlTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class JtlTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<JTL.resource.jtl.mopp.JtlTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<JTL.resource.jtl.mopp.JtlTaskItem> foundItems = new java.util.ArrayList<JTL.resource.jtl.mopp.JtlTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new JTL.resource.jtl.mopp.JtlTaskItem(keyword, message, localLine, offset, end));
