 ...
 * contains <code>ASPT.resource.aspt.IAsptTextDiagnostic</code>s it uses the more
 * precise information of this extended diagnostic type.
 */
public class AsptMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = ASPT.resource.aspt.mopp.AsptPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<ASPT.resource.aspt.IAsptCommand<Object>> commands = new java.util.ArrayList<ASPT.resource.aspt.IAsptCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(ASPT.resource.aspt.IAsptCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<ASPT.resource.aspt.IAsptCommand<Object>> commandsToProcess = new java.util.ArrayList<ASPT.resource.aspt.IAsptCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (ASPT.resource.aspt.IAsptCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, ASPT.resource.aspt.IAsptTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final ASPT.resource.aspt.IAsptTextDiagnostic diagnostic) {
		final ASPT.resource.aspt.IAsptProblem problem = diagnostic.getProblem();
		ASPT.resource.aspt.AsptEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new ASPT.resource.aspt.IAsptCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == ASPT.resource.aspt.AsptEProblemSeverity.ERROR) {
					...
					ASPT.resource.aspt.IAsptTextDiagnostic textDiagnostic = (ASPT.resource.aspt.IAsptTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof ASPT.resource.aspt.mopp.AsptResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((ASPT.resource.aspt.mopp.AsptResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<ASPT.resource.aspt.IAsptQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, ASPT.resource.aspt.util.AsptStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (ASPT.resource.aspt.AsptEProblemType nextType : ASPT.resource.aspt.AsptEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, ASPT.resource.aspt.AsptEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(ASPT.resource.aspt.AsptEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(ASPT.resource.aspt.AsptEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new ASPT.resource.aspt.util.AsptRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new ASPT.resource.aspt.IAsptCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					ASPT.resource.aspt.mopp.AsptPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
