...
public abstract class AsptANTLRParserBase extends org.antlr.runtime3_4_0.Parser implements ASPT.resource.aspt.IAsptTextParser {
	 ...
	 * result object.
	 */
	protected java.util.Collection<ASPT.resource.aspt.IAsptCommand<ASPT.resource.aspt.IAsptTextResource>> postParseCommands;
	 ...
	 */
	private ASPT.resource.aspt.mopp.AsptTokenResolveResult tokenResolveResult = new ASPT.resource.aspt.mopp.AsptTokenResolveResult();
	
	protected ASPT.resource.aspt.mopp.AsptMetaInformation metaInformation = new ASPT.resource.aspt.mopp.AsptMetaInformation();
	
	public AsptANTLRParserBase(org.antlr.runtime3_4_0.TokenStream input) {
		super(input);
	}
	
	public AsptANTLRParserBase(org.antlr.runtime3_4_0.TokenStream input, org.antlr.runtime3_4_0.RecognizerSharedState state) {
		super(input, state);
	}
	
	protected void retrieveLayoutInformation(org.eclipse.emf.ecore.EObject element, ASPT.resource.aspt.grammar.AsptSyntaxElement syntaxElement, Object object, boolean ignoreTokensAfterLastVisibleToken) {
		...
		isElementToStore |= syntaxElement instanceof ASPT.resource.aspt.grammar.AsptPlaceholder;
		isElementToStore |= syntaxElement instanceof ASPT.resource.aspt.grammar.AsptKeyword;
		isElementToStore |= syntaxElement instanceof ASPT.resource.aspt.grammar.AsptEnumerationTerminal;
		isElementToStore |= syntaxElement instanceof ASPT.resource.aspt.grammar.AsptBooleanTerminal;
		if (!isElementToStore) {
			return;
		}
		ASPT.resource.aspt.mopp.AsptLayoutInformationAdapter layoutInformationAdapter = getLayoutInformationAdapter(element);
		...
		layoutInformationAdapter.addLayoutInformation(new ASPT.resource.aspt.mopp.AsptLayoutInformation(syntaxElement, object, offset, hiddenTokenText.toString(), visibleTokenText.toString()));
	...
	protected <ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> void registerContextDependentProxy(final ASPT.resource.aspt.mopp.AsptContextDependentURIFragmentFactory<ContainerType, ReferenceType> factory, final ContainerType container, final org.eclipse.emf.ecore.EReference reference, final String id, final org.eclipse.emf.ecore.EObject proxy) {
		final int position;
		if (reference.isMany()) {
	...
	protected String formatTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = ASPT.resource.aspt.util.AsptStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
	...
	public void setOptions(java.util.Map<?,?> options) {
		...
		if (this.options.containsKey(ASPT.resource.aspt.IAsptOptions.DISABLE_LOCATION_MAP)) {
			this.disableLocationMap = true;
		}
		if (this.options.containsKey(ASPT.resource.aspt.IAsptOptions.DISABLE_LAYOUT_INFORMATION_RECORDING)) {
	...
	protected void addMapEntry(org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EStructuralFeature structuralFeature, ASPT.resource.aspt.mopp.AsptDummyEObject dummy) {
		Object value = element.eGet(structuralFeature);
		Object mapKey = dummy.getValueByName("key");
		Object mapValue = dummy.getValueByName("value");
		if (value instanceof org.eclipse.emf.common.util.EMap<?, ?>) {
			org.eclipse.emf.common.util.EMap<Object, Object> valueMap = ASPT.resource.aspt.util.AsptMapUtil.castToEMap(value);
	...
	protected org.eclipse.emf.ecore.EObject apply(org.eclipse.emf.ecore.EObject target, java.util.List<org.eclipse.emf.ecore.EObject> dummyEObjects) {
		...
		for (org.eclipse.emf.ecore.EObject object : dummyEObjects) {
			assert(object instanceof ASPT.resource.aspt.mopp.AsptDummyEObject);
			ASPT.resource.aspt.mopp.AsptDummyEObject dummy = (ASPT.resource.aspt.mopp.AsptDummyEObject) object;
	...
	protected ASPT.resource.aspt.mopp.AsptTokenResolveResult getFreshTokenResolveResult() {
		tokenResolveResult.clear();
		return tokenResolveResult;
	}
	
	protected ASPT.resource.aspt.mopp.AsptReferenceResolverSwitch getReferenceResolverSwitch() {
		ASPT.resource.aspt.mopp.AsptReferenceResolverSwitch resolverSwitch = (ASPT.resource.aspt.mopp.AsptReferenceResolverSwitch) metaInformation.getReferenceResolverSwitch();
