...
/**
 * The AsptTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * AsptTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class AsptTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<ASPT.resource.aspt.mopp.AsptTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<ASPT.resource.aspt.mopp.AsptTaskItem> foundItems = new java.util.ArrayList<ASPT.resource.aspt.mopp.AsptTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new ASPT.resource.aspt.mopp.AsptTaskItem(keyword, message, localLine, offset, end));
