 ...
 */
public class AsptMinimalModelHelper {
	
	private final static ASPT.resource.aspt.util.AsptEClassUtil eClassUtil = new ASPT.resource.aspt.util.AsptEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + ASPT.resource.aspt.util.AsptStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = ASPT.resource.aspt.util.AsptListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + ASPT.resource.aspt.util.AsptStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = ASPT.resource.aspt.util.AsptListUtil.castListUnchecked(value);
