...
public class AsptDebugThread extends ASPT.resource.aspt.debug.AsptDebugElement implements org.eclipse.debug.core.model.IThread, ASPT.resource.aspt.debug.IAsptDebugEventListener {
	
	private boolean suspended = false;
	private ASPT.resource.aspt.debug.AsptDebugTarget debugTarget;
	
	public AsptDebugThread(ASPT.resource.aspt.debug.AsptDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			ASPT.resource.aspt.debug.AsptDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = ASPT.resource.aspt.util.AsptStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new ASPT.resource.aspt.debug.AsptStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public ASPT.resource.aspt.debug.AsptDebugTarget getTarget() {
		return debugTarget;
	}
