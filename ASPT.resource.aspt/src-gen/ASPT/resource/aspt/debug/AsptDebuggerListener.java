 ...
 * code).
 */
public class AsptDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private ASPT.resource.aspt.debug.AbstractAsptDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, ASPT.resource.aspt.util.AsptPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, ASPT.resource.aspt.util.AsptPair<String, Object>>();
	...
	public AsptDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		ASPT.resource.aspt.debug.AsptDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = ASPT.resource.aspt.util.AsptStringUtil.encode('#', stack);
				ASPT.resource.aspt.debug.AsptDebugMessage message = new ASPT.resource.aspt.debug.AsptDebugMessage(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				ASPT.resource.aspt.debug.AsptDebugMessage message = new ASPT.resource.aspt.debug.AsptDebugMessage(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.GET_VARIABLES)) {
				...
				ASPT.resource.aspt.debug.AsptDebugMessage message = new ASPT.resource.aspt.debug.AsptDebugMessage(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return ASPT.resource.aspt.util.AsptStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		ASPT.resource.aspt.util.AsptPair<String, Object> pair = new ASPT.resource.aspt.util.AsptPair<String, Object>(name, value);
	...
	public ASPT.resource.aspt.debug.AbstractAsptDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(ASPT.resource.aspt.debug.AbstractAsptDebuggable debuggable) {
