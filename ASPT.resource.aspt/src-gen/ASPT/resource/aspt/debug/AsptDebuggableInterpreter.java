 ...
 */
public class AsptDebuggableInterpreter<ResultType, ContextType> extends ASPT.resource.aspt.debug.AbstractAsptDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private ASPT.resource.aspt.util.AbstractAsptInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private ASPT.resource.aspt.IAsptCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public AsptDebuggableInterpreter(ASPT.resource.aspt.util.AbstractAsptInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new ASPT.resource.aspt.IAsptInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = ASPT.resource.aspt.util.AsptStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public ASPT.resource.aspt.util.AbstractAsptInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		ASPT.resource.aspt.IAsptLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private ASPT.resource.aspt.IAsptLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		ASPT.resource.aspt.IAsptCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(ASPT.resource.aspt.debug.EAsptDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = ASPT.resource.aspt.util.AsptEObjectUtil.getDepth(current);
		stopCondition = new ASPT.resource.aspt.IAsptCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = ASPT.resource.aspt.util.AsptEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = ASPT.resource.aspt.util.AsptEObjectUtil.getDepth(current) - 1;
		stopCondition = new ASPT.resource.aspt.IAsptCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
