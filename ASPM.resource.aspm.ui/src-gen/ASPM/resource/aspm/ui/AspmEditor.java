...
/**
 * A text editor for 'aspm' models.
 * <p>
 * This editor has id <code>ASPM.resource.aspm.ui.AspmEditor</code>
 * The editor's context menu has id <code>ASPM.resource.aspm.EditorContext</code>. 
 * The editor's ruler context menu has id
 * <code>ASPM.resource.aspm.EditorRuler</code>.
 * </p>
 */
public class AspmEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, ASPM.resource.aspm.IAspmResourceProvider, ASPM.resource.aspm.ui.IAspmBracketHandlerProvider, ASPM.resource.aspm.ui.IAspmAnnotationModelProvider {
	
	private ASPM.resource.aspm.ui.AspmHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private ASPM.resource.aspm.ui.AspmCodeFoldingManager codeFoldingManager;
	private ASPM.resource.aspm.ui.AspmBackgroundParsingStrategy bgParsingStrategy = new ASPM.resource.aspm.ui.AspmBackgroundParsingStrategy();
	private java.util.Collection<ASPM.resource.aspm.IAspmBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<ASPM.resource.aspm.IAspmBackgroundParsingListener>();
	private ASPM.resource.aspm.ui.AspmColorManager colorManager = new ASPM.resource.aspm.ui.AspmColorManager();
	private ASPM.resource.aspm.ui.AspmOutlinePage outlinePage;
	...
	private ASPM.resource.aspm.ui.AspmPropertySheetPage propertySheetPage;
	...
	private ASPM.resource.aspm.ui.IAspmBracketHandler bracketHandler;
	...
	public AspmEditor() {
		super();
		setSourceViewerConfiguration(new ASPM.resource.aspm.ui.AspmSourceViewerConfiguration(this, this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), AspmEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								ASPM.resource.aspm.IAspmTextResource currentResource = getResource();
				...
				ASPM.resource.aspm.ui.AspmUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("ASPM.resource.aspm.EditorContext");
		setRulerContextMenuId("ASPM.resource.aspm.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new ASPM.resource.aspm.ui.AspmHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new ASPM.resource.aspm.ui.AspmCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		ASPM.resource.aspm.mopp.AspmNature.activate(inputFile.getProject());
		...
		ASPM.resource.aspm.IAspmTextResource loadedResource = (ASPM.resource.aspm.IAspmTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				ASPM.resource.aspm.IAspmTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof ASPM.resource.aspm.IAspmTextResource) {
					setResource((ASPM.resource.aspm.IAspmTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					ASPM.resource.aspm.ui.AspmUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the AspmEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				ASPM.resource.aspm.ui.AspmUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(ASPM.resource.aspm.IAspmTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new ASPM.resource.aspm.ui.AspmOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new ASPM.resource.aspm.ui.AspmPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			ASPM.resource.aspm.IAspmTextResource textResource = (ASPM.resource.aspm.IAspmTextResource) element.eResource();
			...
			ASPM.resource.aspm.IAspmTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				ASPM.resource.aspm.IAspmTextToken token = lexer.getNextToken();
			...
			ASPM.resource.aspm.ui.AspmUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!AspmEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(ASPM.resource.aspm.IAspmBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (ASPM.resource.aspm.IAspmBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public ASPM.resource.aspm.ui.IAspmBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(ASPM.resource.aspm.ui.IAspmBracketHandler bracketHandler) {
	...
	private boolean setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
		if (selection instanceof org.eclipse.jface.viewers.IStructuredSelection) {
			...
			if (object instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (!(resource instanceof ASPM.resource.aspm.IAspmTextResource)) {
					return false;
				}
				ASPM.resource.aspm.IAspmTextResource textResource = (ASPM.resource.aspm.IAspmTextResource) resource;
				ASPM.resource.aspm.IAspmLocationMap locationMap = textResource.getLocationMap();
				int destination = locationMap.getCharStart(element);
				if (destination < 0) {
					destination = 0;
