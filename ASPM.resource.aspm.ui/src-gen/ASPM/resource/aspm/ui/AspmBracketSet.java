...
public class AspmBracketSet {
	...
	private java.util.ArrayList<ASPM.resource.aspm.IAspmBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements ASPM.resource.aspm.IAspmBracketPair {
	 ...
	 */
	private class ClosingListener implements ASPM.resource.aspm.ui.IAspmBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				ASPM.resource.aspm.IAspmBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public AspmBracketSet(ASPM.resource.aspm.ui.AspmEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new ASPM.resource.aspm.mopp.AspmMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<ASPM.resource.aspm.IAspmBracketPair>();
	 ...
	 */
	public ASPM.resource.aspm.IAspmBracketPair getBracketPair(String opening, String closing) {
		for (ASPM.resource.aspm.IAspmBracketPair bracketPair : bracketPairs) {
	...
	public ASPM.resource.aspm.IAspmBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(ASPM.resource.aspm.IAspmBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + ASPM.resource.aspm.ui.AspmPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public ASPM.resource.aspm.IAspmBracketPair remove(String opening, String closing) {
		for (ASPM.resource.aspm.IAspmBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<ASPM.resource.aspm.IAspmBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, ASPM.resource.aspm.ui.AspmPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, ASPM.resource.aspm.ui.AspmPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
