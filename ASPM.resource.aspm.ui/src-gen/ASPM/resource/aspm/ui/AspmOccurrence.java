 ...
 */
public class AspmOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "ASPM.resource.aspm.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "ASPM.resource.aspm.ui.occurences.declaration";
	...
	private ASPM.resource.aspm.ui.AspmTokenScanner tokenScanner;
	 ...
	 */
	public AspmOccurrence(ASPM.resource.aspm.IAspmTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, ASPM.resource.aspm.ui.AspmTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new ASPM.resource.aspm.mopp.AspmMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(ASPM.resource.aspm.ui.AspmBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, ASPM.resource.aspm.ui.AspmPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, ASPM.resource.aspm.ui.AspmPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, ASPM.resource.aspm.ui.AspmPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == ASPM.resource.aspm.ui.AspmPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(ASPM.resource.aspm.ui.AspmOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(ASPM.resource.aspm.ui.AspmOccurrence.DECLARATION_ANNOTATION_ID);
	}
