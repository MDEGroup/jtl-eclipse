...
/**
 * A text editor for 'aspt' models.
 * <p>
 * This editor has id <code>ASPT.resource.aspt.ui.AsptEditor</code>
 * The editor's context menu has id <code>ASPT.resource.aspt.EditorContext</code>. 
 * The editor's ruler context menu has id
 * <code>ASPT.resource.aspt.EditorRuler</code>.
 * </p>
 */
public class AsptEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, ASPT.resource.aspt.IAsptResourceProvider, ASPT.resource.aspt.ui.IAsptBracketHandlerProvider, ASPT.resource.aspt.ui.IAsptAnnotationModelProvider {
	
	private ASPT.resource.aspt.ui.AsptHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private ASPT.resource.aspt.ui.AsptCodeFoldingManager codeFoldingManager;
	private ASPT.resource.aspt.ui.AsptBackgroundParsingStrategy bgParsingStrategy = new ASPT.resource.aspt.ui.AsptBackgroundParsingStrategy();
	private java.util.Collection<ASPT.resource.aspt.IAsptBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<ASPT.resource.aspt.IAsptBackgroundParsingListener>();
	private ASPT.resource.aspt.ui.AsptColorManager colorManager = new ASPT.resource.aspt.ui.AsptColorManager();
	private ASPT.resource.aspt.ui.AsptOutlinePage outlinePage;
	...
	private ASPT.resource.aspt.ui.AsptPropertySheetPage propertySheetPage;
	...
	private ASPT.resource.aspt.ui.IAsptBracketHandler bracketHandler;
	...
	public AsptEditor() {
		super();
		setSourceViewerConfiguration(new ASPT.resource.aspt.ui.AsptSourceViewerConfiguration(this, this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), AsptEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								ASPT.resource.aspt.IAsptTextResource currentResource = getResource();
				...
				ASPT.resource.aspt.ui.AsptUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("ASPT.resource.aspt.EditorContext");
		setRulerContextMenuId("ASPT.resource.aspt.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new ASPT.resource.aspt.ui.AsptHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new ASPT.resource.aspt.ui.AsptCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		ASPT.resource.aspt.mopp.AsptNature.activate(inputFile.getProject());
		...
		ASPT.resource.aspt.IAsptTextResource loadedResource = (ASPT.resource.aspt.IAsptTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				ASPT.resource.aspt.IAsptTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof ASPT.resource.aspt.IAsptTextResource) {
					setResource((ASPT.resource.aspt.IAsptTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					ASPT.resource.aspt.ui.AsptUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the AsptEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				ASPT.resource.aspt.ui.AsptUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(ASPT.resource.aspt.IAsptTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new ASPT.resource.aspt.ui.AsptOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new ASPT.resource.aspt.ui.AsptPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			ASPT.resource.aspt.IAsptTextResource textResource = (ASPT.resource.aspt.IAsptTextResource) element.eResource();
			ASPT.resource.aspt.IAsptLocationMap locationMap = textResource.getLocationMap();
			int destination = locationMap.getCharStart(element);
			int length = locationMap.getCharEnd(element) + 1 - destination;
			
			ASPT.resource.aspt.IAsptTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				ASPT.resource.aspt.IAsptTextToken token = lexer.getNextToken();
			...
			ASPT.resource.aspt.ui.AsptUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!AsptEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(ASPT.resource.aspt.IAsptBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (ASPT.resource.aspt.IAsptBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public ASPT.resource.aspt.ui.IAsptBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(ASPT.resource.aspt.ui.IAsptBracketHandler bracketHandler) {
	...
	private boolean setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
		if (selection instanceof org.eclipse.jface.viewers.IStructuredSelection) {
			...
			if (object instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (!(resource instanceof ASPT.resource.aspt.IAsptTextResource)) {
					return false;
				}
				ASPT.resource.aspt.IAsptTextResource textResource = (ASPT.resource.aspt.IAsptTextResource) resource;
				ASPT.resource.aspt.IAsptLocationMap locationMap = textResource.getLocationMap();
				int destination = locationMap.getCharStart(element);
				if (destination < 0) {
					destination = 0;
