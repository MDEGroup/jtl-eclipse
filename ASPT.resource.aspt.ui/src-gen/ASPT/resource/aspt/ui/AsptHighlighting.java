 ...
 */
public class AsptHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static ASPT.resource.aspt.ui.AsptPositionHelper positionHelper = new ASPT.resource.aspt.ui.AsptPositionHelper();
	...
	private ASPT.resource.aspt.ui.AsptTokenScanner scanner;
	...
	private ASPT.resource.aspt.ui.AsptEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private ASPT.resource.aspt.ui.AsptOccurrence occurrence;
	private ASPT.resource.aspt.ui.AsptBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, ASPT.resource.aspt.IAsptBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public AsptHighlighting(ASPT.resource.aspt.IAsptTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, ASPT.resource.aspt.ui.AsptColorManager colorManager, ASPT.resource.aspt.ui.AsptEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = ASPT.resource.aspt.ui.AsptUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new ASPT.resource.aspt.ui.AsptTokenScanner(textResource, colorManager);
		occurrence = new ASPT.resource.aspt.ui.AsptOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new ASPT.resource.aspt.ui.AsptBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(ASPT.resource.aspt.ui.AsptPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, ASPT.resource.aspt.ui.AsptPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(ASPT.resource.aspt.ui.AsptEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, ASPT.resource.aspt.ui.AsptPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, ASPT.resource.aspt.ui.AsptPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(ASPT.resource.aspt.ui.AsptPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new ASPT.resource.aspt.ui.AsptEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
