 ...
 */
public class AsptOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "ASPT.resource.aspt.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "ASPT.resource.aspt.ui.occurences.declaration";
	...
	private ASPT.resource.aspt.ui.AsptTokenScanner tokenScanner;
	...
	private ASPT.resource.aspt.IAsptTextResource textResource;
	 ...
	 */
	public AsptOccurrence(ASPT.resource.aspt.IAsptTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, ASPT.resource.aspt.ui.AsptTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new ASPT.resource.aspt.mopp.AsptMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(ASPT.resource.aspt.ui.AsptBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, ASPT.resource.aspt.ui.AsptPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, ASPT.resource.aspt.ui.AsptPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, ASPT.resource.aspt.ui.AsptPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == ASPT.resource.aspt.ui.AsptPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(ASPT.resource.aspt.ui.AsptOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(ASPT.resource.aspt.ui.AsptOccurrence.DECLARATION_ANNOTATION_ID);
	}
