...
public class AsptBracketSet {
	...
	private java.util.ArrayList<ASPT.resource.aspt.IAsptBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements ASPT.resource.aspt.IAsptBracketPair {
	 ...
	 */
	private class ClosingListener implements ASPT.resource.aspt.ui.IAsptBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				ASPT.resource.aspt.IAsptBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public AsptBracketSet(ASPT.resource.aspt.ui.AsptEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new ASPT.resource.aspt.mopp.AsptMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<ASPT.resource.aspt.IAsptBracketPair>();
	 ...
	 */
	public ASPT.resource.aspt.IAsptBracketPair getBracketPair(String opening, String closing) {
		for (ASPT.resource.aspt.IAsptBracketPair bracketPair : bracketPairs) {
	...
	public ASPT.resource.aspt.IAsptBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(ASPT.resource.aspt.IAsptBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + ASPT.resource.aspt.ui.AsptPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public ASPT.resource.aspt.IAsptBracketPair remove(String opening, String closing) {
		for (ASPT.resource.aspt.IAsptBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<ASPT.resource.aspt.IAsptBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, ASPT.resource.aspt.ui.AsptPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, ASPT.resource.aspt.ui.AsptPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
