 ...
 */
public class AsptBackgroundParsingStrategy {
	 ...
	 */
	public void parse(org.eclipse.jface.text.DocumentEvent event, final ASPT.resource.aspt.IAsptTextResource resource, final ASPT.resource.aspt.ui.AsptEditor editor) {
	 ...
	 * Schedules a task for background parsing that will be started after a delay.
	 */
	public void parse(org.eclipse.jface.text.IDocument document, final ASPT.resource.aspt.IAsptTextResource resource, final ASPT.resource.aspt.ui.AsptEditor editor, long delay) {
	 ...
	 * Schedules a task for background parsing that will be started after a delay.
	 */
	public void parse(final String contents, final ASPT.resource.aspt.IAsptTextResource resource, final ASPT.resource.aspt.ui.AsptEditor editor, long delay) {
		if (resource == null) {
			return;
		}
		if (contents == null) {
			...
			while (newContents != null ) {
				while (newContents != null) {
					try {
						...
						if (resource instanceof ASPT.resource.aspt.mopp.AsptResource) {
							ASPT.resource.aspt.mopp.AsptResource concreteResource = (ASPT.resource.aspt.mopp.AsptResource) resource;
