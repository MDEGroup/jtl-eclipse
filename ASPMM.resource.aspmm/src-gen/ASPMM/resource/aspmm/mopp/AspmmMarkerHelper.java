 ...
 * contains <code>ASPMM.resource.aspmm.IAspmmTextDiagnostic</code>s it uses the
 * more precise information of this extended diagnostic type.
 */
public class AspmmMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = ASPMM.resource.aspmm.mopp.AspmmPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<ASPMM.resource.aspmm.IAspmmCommand<Object>> commands = new java.util.ArrayList<ASPMM.resource.aspmm.IAspmmCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(ASPMM.resource.aspmm.IAspmmCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<ASPMM.resource.aspmm.IAspmmCommand<Object>> commandsToProcess = new java.util.ArrayList<ASPMM.resource.aspmm.IAspmmCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (ASPMM.resource.aspmm.IAspmmCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, ASPMM.resource.aspmm.IAspmmTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final ASPMM.resource.aspmm.IAspmmTextDiagnostic diagnostic) {
		final ASPMM.resource.aspmm.IAspmmProblem problem = diagnostic.getProblem();
		ASPMM.resource.aspmm.AspmmEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new ASPMM.resource.aspmm.IAspmmCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == ASPMM.resource.aspmm.AspmmEProblemSeverity.ERROR) {
					...
					ASPMM.resource.aspmm.IAspmmTextDiagnostic textDiagnostic = (ASPMM.resource.aspmm.IAspmmTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof ASPMM.resource.aspmm.mopp.AspmmResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((ASPMM.resource.aspmm.mopp.AspmmResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<ASPMM.resource.aspmm.IAspmmQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, ASPMM.resource.aspmm.util.AspmmStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (ASPMM.resource.aspmm.AspmmEProblemType nextType : ASPMM.resource.aspmm.AspmmEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, ASPMM.resource.aspmm.AspmmEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(ASPMM.resource.aspmm.AspmmEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(ASPMM.resource.aspmm.AspmmEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new ASPMM.resource.aspmm.util.AspmmRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new ASPMM.resource.aspmm.IAspmmCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					ASPMM.resource.aspmm.mopp.AspmmPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
