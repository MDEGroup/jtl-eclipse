...
/**
 * The AspmmTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * AspmmTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class AspmmTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<ASPMM.resource.aspmm.mopp.AspmmTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<ASPMM.resource.aspmm.mopp.AspmmTaskItem> foundItems = new java.util.ArrayList<ASPMM.resource.aspmm.mopp.AspmmTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new ASPMM.resource.aspmm.mopp.AspmmTaskItem(keyword, message, localLine, offset, end));
