...
public class AspmmLayoutUtil {
	...
	public void transferLayoutInformationToModel(org.eclipse.emf.ecore.EObject element) {
		...
		for (java.util.Iterator<ASPMM.resource.aspmm.mopp.AspmmLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			ASPMM.resource.aspmm.mopp.AspmmLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(org.eclipse.emf.ecore.EObject element) {
		...
		if (layoutReference != null) {
			...
			for (java.util.Iterator<org.eclipse.emf.ecore.EObject> i = list.iterator(); i.hasNext(); ) {
				...
				ASPMM.resource.aspmm.mopp.AspmmLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public org.eclipse.emf.ecore.EObject createLayoutInformationModelElement(ASPMM.resource.aspmm.mopp.AspmmLayoutInformation layoutInformation, org.eclipse.emf.ecore.EPackage layoutPackage) {
		...
		ASPMM.resource.aspmm.grammar.AspmmSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), ASPMM.resource.aspmm.grammar.AspmmGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public ASPMM.resource.aspmm.mopp.AspmmLayoutInformation createLayoutInformation(org.eclipse.emf.ecore.EObject layoutInformationModelElement) {
		Object object = null;
		...
		ASPMM.resource.aspmm.grammar.AspmmSyntaxElement syntaxElement = ASPMM.resource.aspmm.grammar.AspmmGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof ASPMM.resource.aspmm.grammar.AspmmPlaceholder) {
			ASPMM.resource.aspmm.grammar.AspmmPlaceholder placeholder = (ASPMM.resource.aspmm.grammar.AspmmPlaceholder) syntaxElement;
			ASPMM.resource.aspmm.IAspmmTokenResolver tokenResolver = new ASPMM.resource.aspmm.mopp.AspmmTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			ASPMM.resource.aspmm.IAspmmTokenResolveResult result = new ASPMM.resource.aspmm.mopp.AspmmTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new ASPMM.resource.aspmm.mopp.AspmmLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public ASPMM.resource.aspmm.mopp.AspmmLayoutInformationAdapter getLayoutInformationAdapter(org.eclipse.emf.ecore.EObject element) {
