 ...
 */
public class AspmmMinimalModelHelper {
	
	private final static ASPMM.resource.aspmm.util.AspmmEClassUtil eClassUtil = new ASPMM.resource.aspmm.util.AspmmEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + ASPMM.resource.aspmm.util.AspmmStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = ASPMM.resource.aspmm.util.AspmmListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + ASPMM.resource.aspmm.util.AspmmStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = ASPMM.resource.aspmm.util.AspmmListUtil.castListUnchecked(value);
