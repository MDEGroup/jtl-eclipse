 ...
 */
public class AspmmDebuggableInterpreter<ResultType, ContextType> extends ASPMM.resource.aspmm.debug.AbstractAspmmDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private ASPMM.resource.aspmm.util.AbstractAspmmInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private ASPMM.resource.aspmm.IAspmmCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public AspmmDebuggableInterpreter(ASPMM.resource.aspmm.util.AbstractAspmmInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new ASPMM.resource.aspmm.IAspmmInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = ASPMM.resource.aspmm.util.AspmmStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public ASPMM.resource.aspmm.util.AbstractAspmmInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		ASPMM.resource.aspmm.IAspmmLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private ASPMM.resource.aspmm.IAspmmLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		ASPMM.resource.aspmm.IAspmmCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = ASPMM.resource.aspmm.util.AspmmEObjectUtil.getDepth(current);
		stopCondition = new ASPMM.resource.aspmm.IAspmmCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = ASPMM.resource.aspmm.util.AspmmEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = ASPMM.resource.aspmm.util.AspmmEObjectUtil.getDepth(current) - 1;
		stopCondition = new ASPMM.resource.aspmm.IAspmmCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
