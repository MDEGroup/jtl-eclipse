 ...
 * code).
 */
public class AspmmDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private ASPMM.resource.aspmm.debug.AbstractAspmmDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, ASPMM.resource.aspmm.util.AspmmPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, ASPMM.resource.aspmm.util.AspmmPair<String, Object>>();
	...
	public AspmmDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		ASPMM.resource.aspmm.debug.AspmmDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = ASPMM.resource.aspmm.util.AspmmStringUtil.encode('#', stack);
				ASPMM.resource.aspmm.debug.AspmmDebugMessage message = new ASPMM.resource.aspmm.debug.AspmmDebugMessage(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				ASPMM.resource.aspmm.debug.AspmmDebugMessage message = new ASPMM.resource.aspmm.debug.AspmmDebugMessage(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.GET_VARIABLES)) {
				...
				ASPMM.resource.aspmm.debug.AspmmDebugMessage message = new ASPMM.resource.aspmm.debug.AspmmDebugMessage(ASPMM.resource.aspmm.debug.EAspmmDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return ASPMM.resource.aspmm.util.AspmmStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		ASPMM.resource.aspmm.util.AspmmPair<String, Object> pair = new ASPMM.resource.aspmm.util.AspmmPair<String, Object>(name, value);
	...
	public ASPMM.resource.aspmm.debug.AbstractAspmmDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(ASPMM.resource.aspmm.debug.AbstractAspmmDebuggable debuggable) {
