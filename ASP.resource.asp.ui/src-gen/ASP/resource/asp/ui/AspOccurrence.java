 ...
 */
public class AspOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "ASP.resource.asp.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "ASP.resource.asp.ui.occurences.declaration";
	
	private final static ASP.resource.asp.ui.AspPositionHelper positionHelper = new ASP.resource.asp.ui.AspPositionHelper();
	
	private ASP.resource.asp.ui.AspTokenScanner tokenScanner;
	 ...
	 */
	public AspOccurrence(ASP.resource.asp.IAspTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, ASP.resource.asp.ui.AspTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new ASP.resource.asp.mopp.AspMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(ASP.resource.asp.ui.AspBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, ASP.resource.asp.ui.AspPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, ASP.resource.asp.ui.AspPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, ASP.resource.asp.ui.AspPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == ASP.resource.asp.ui.AspPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(ASP.resource.asp.ui.AspOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(ASP.resource.asp.ui.AspOccurrence.DECLARATION_ANNOTATION_ID);
	}
