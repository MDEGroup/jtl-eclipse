 ...
 */
public class AspHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static ASP.resource.asp.ui.AspPositionHelper positionHelper = new ASP.resource.asp.ui.AspPositionHelper();
	...
	private ASP.resource.asp.ui.AspTokenScanner scanner;
	...
	private ASP.resource.asp.ui.AspEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private ASP.resource.asp.ui.AspOccurrence occurrence;
	private ASP.resource.asp.ui.AspBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, ASP.resource.asp.IAspBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public AspHighlighting(ASP.resource.asp.IAspTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, ASP.resource.asp.ui.AspColorManager colorManager, ASP.resource.asp.ui.AspEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = ASP.resource.asp.ui.AspUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new ASP.resource.asp.ui.AspTokenScanner(textResource, colorManager);
		occurrence = new ASP.resource.asp.ui.AspOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new ASP.resource.asp.ui.AspBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(ASP.resource.asp.ui.AspPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, ASP.resource.asp.ui.AspPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(ASP.resource.asp.ui.AspEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, ASP.resource.asp.ui.AspPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, ASP.resource.asp.ui.AspPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(ASP.resource.asp.ui.AspPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new ASP.resource.asp.ui.AspEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
