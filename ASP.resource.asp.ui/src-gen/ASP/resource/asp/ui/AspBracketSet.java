...
public class AspBracketSet {
	...
	private java.util.ArrayList<ASP.resource.asp.IAspBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements ASP.resource.asp.IAspBracketPair {
	 ...
	 */
	private class ClosingListener implements ASP.resource.asp.ui.IAspBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				ASP.resource.asp.IAspBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public AspBracketSet(ASP.resource.asp.ui.AspEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new ASP.resource.asp.mopp.AspMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<ASP.resource.asp.IAspBracketPair>();
	 ...
	 */
	public ASP.resource.asp.IAspBracketPair getBracketPair(String opening, String closing) {
		for (ASP.resource.asp.IAspBracketPair bracketPair : bracketPairs) {
	...
	public ASP.resource.asp.IAspBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(ASP.resource.asp.IAspBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + ASP.resource.asp.ui.AspPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public ASP.resource.asp.IAspBracketPair remove(String opening, String closing) {
		for (ASP.resource.asp.IAspBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<ASP.resource.asp.IAspBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, ASP.resource.asp.ui.AspPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, ASP.resource.asp.ui.AspPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
