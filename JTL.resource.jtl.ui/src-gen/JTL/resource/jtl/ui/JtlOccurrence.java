 ...
 */
public class JtlOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "JTL.resource.jtl.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "JTL.resource.jtl.ui.occurences.declaration";
	
	private final static JTL.resource.jtl.ui.JtlPositionHelper positionHelper = new JTL.resource.jtl.ui.JtlPositionHelper();
	
	private JTL.resource.jtl.ui.JtlTokenScanner tokenScanner;
	 ...
	 */
	public JtlOccurrence(JTL.resource.jtl.IJtlTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, JTL.resource.jtl.ui.JtlTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new JTL.resource.jtl.mopp.JtlMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(JTL.resource.jtl.ui.JtlBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, JTL.resource.jtl.ui.JtlPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, JTL.resource.jtl.ui.JtlPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, JTL.resource.jtl.ui.JtlPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == JTL.resource.jtl.ui.JtlPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(JTL.resource.jtl.ui.JtlOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(JTL.resource.jtl.ui.JtlOccurrence.DECLARATION_ANNOTATION_ID);
	}
