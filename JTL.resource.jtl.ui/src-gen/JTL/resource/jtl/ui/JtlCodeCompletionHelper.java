 ...
 */
public class JtlCodeCompletionHelper {
	
	private JTL.resource.jtl.mopp.JtlAttributeValueProvider attributeValueProvider = new JTL.resource.jtl.mopp.JtlAttributeValueProvider();
	
	private JTL.resource.jtl.IJtlMetaInformation metaInformation = new JTL.resource.jtl.mopp.JtlMetaInformation();
	 ...
	 * @return
	 */
	public JTL.resource.jtl.ui.JtlCompletionProposal[] computeCompletionProposals(JTL.resource.jtl.IJtlTextResource originalResource, String content, int cursorOffset) {
		...
		JTL.resource.jtl.IJtlTextResource resource = (JTL.resource.jtl.IJtlTextResource) resourceSet.createResource(originalResource.getURI());
		java.io.ByteArrayInputStream inputStream = new java.io.ByteArrayInputStream(content.getBytes());
		JTL.resource.jtl.IJtlMetaInformation metaInformation = resource.getMetaInformation();
		JTL.resource.jtl.IJtlTextParser parser = metaInformation.createParser(inputStream, null);
		JTL.resource.jtl.mopp.JtlExpectedTerminal[] expectedElements = parseToExpectedElements(parser, resource, cursorOffset);
		if (expectedElements == null) {
			return new JTL.resource.jtl.ui.JtlCompletionProposal[0];
		}
		if (expectedElements.length == 0) {
			return new JTL.resource.jtl.ui.JtlCompletionProposal[0];
		}
		java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> expectedAfterCursor = java.util.Arrays.asList(getElementsExpectedAt(expectedElements, cursorOffset));
		java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> expectedBeforeCursor = java.util.Arrays.asList(getElementsExpectedAt(expectedElements, cursorOffset - 1));
		...
		java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> allProposals = new java.util.LinkedHashSet<JTL.resource.jtl.ui.JtlCompletionProposal>();
		java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> rightProposals = deriveProposals(expectedAfterCursor, content, resource, cursorOffset);
		java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> leftProposals = deriveProposals(expectedBeforeCursor, content, resource, cursorOffset - 1);
		...
		for (JTL.resource.jtl.ui.JtlCompletionProposal leftProposal : leftProposals) {
		...
		final java.util.List<JTL.resource.jtl.ui.JtlCompletionProposal> sortedProposals = new java.util.ArrayList<JTL.resource.jtl.ui.JtlCompletionProposal>(allProposals);
		java.util.Collections.sort(sortedProposals);
		org.eclipse.emf.ecore.EObject root = null;
		if (!resource.getContents().isEmpty()) {
			root = resource.getContents().get(0);
		}
		for (JTL.resource.jtl.ui.JtlCompletionProposal proposal : sortedProposals) {
			proposal.setRoot(root);
		}
		return sortedProposals.toArray(new JTL.resource.jtl.ui.JtlCompletionProposal[sortedProposals.size()]);
	}
	
	public JTL.resource.jtl.mopp.JtlExpectedTerminal[] parseToExpectedElements(JTL.resource.jtl.IJtlTextParser parser, JTL.resource.jtl.IJtlTextResource resource, int cursorOffset) {
		final java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> expectedElements = parser.parseToExpectedElements(null, resource, cursorOffset);
		if (expectedElements == null) {
			return new JTL.resource.jtl.mopp.JtlExpectedTerminal[0];
		}
		removeDuplicateEntries(expectedElements);
		removeInvalidEntriesAtEnd(expectedElements);
		return expectedElements.toArray(new JTL.resource.jtl.mopp.JtlExpectedTerminal[expectedElements.size()]);
	}
	 ...
	 * the same position.
	 */
	protected void removeDuplicateEntries(java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> expectedElements) {
		...
		java.util.Map<Integer, java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal>> map = new java.util.LinkedHashMap<Integer, java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal>>();
		for (int i = 0; i < size; i++) {
			JTL.resource.jtl.mopp.JtlExpectedTerminal elementAtIndex = expectedElements.get(i);
			int start1 = elementAtIndex.getStartExcludingHiddenTokens();
			java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> list = map.get(start1);
			if (list == null) {
				list = new java.util.ArrayList<JTL.resource.jtl.mopp.JtlExpectedTerminal>();
				map.put(start1, list);
			}
		...
		for (int position : map.keySet()) {
			java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> list = map.get(position);
	 ...
	 */
	protected void removeDuplicateEntriesFromBucket(java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> expectedElements) {
		int size = expectedElements.size();
		for (int i = 0; i < size - 1; i++) {
			JTL.resource.jtl.mopp.JtlExpectedTerminal elementAtIndex = expectedElements.get(i);
			JTL.resource.jtl.IJtlExpectedElement terminal = elementAtIndex.getTerminal();
			for (int j = i + 1; j < size;) {
				JTL.resource.jtl.mopp.JtlExpectedTerminal elementAtNext = expectedElements.get(j);
	...
	protected void removeInvalidEntriesAtEnd(java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> expectedElements) {
		for (int i = 0; i < expectedElements.size() - 1;) {
			JTL.resource.jtl.mopp.JtlExpectedTerminal elementAtIndex = expectedElements.get(i);
			JTL.resource.jtl.mopp.JtlExpectedTerminal elementAtNext = expectedElements.get(i + 1);
			...
			// rule.
			JTL.resource.jtl.grammar.JtlSyntaxElement symtaxElementOfThis = elementAtIndex.getTerminal().getSymtaxElement();
			JTL.resource.jtl.grammar.JtlSyntaxElement symtaxElementOfNext = elementAtNext.getTerminal().getSymtaxElement();
	 ...
	 */
	protected void removeKeywordsEndingBeforeIndex(java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> proposals, int index) {
		java.util.List<JTL.resource.jtl.ui.JtlCompletionProposal> toRemove = new java.util.ArrayList<JTL.resource.jtl.ui.JtlCompletionProposal>();
		for (JTL.resource.jtl.ui.JtlCompletionProposal proposal : proposals) {
			JTL.resource.jtl.mopp.JtlExpectedTerminal expectedTerminal = proposal.getExpectedTerminal();
			JTL.resource.jtl.IJtlExpectedElement terminal = expectedTerminal.getTerminal();
			if (terminal instanceof JTL.resource.jtl.mopp.JtlExpectedCsString) {
				JTL.resource.jtl.mopp.JtlExpectedCsString csString = (JTL.resource.jtl.mopp.JtlExpectedCsString) terminal;
	...
	protected String findPrefix(java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> expectedElements, JTL.resource.jtl.mopp.JtlExpectedTerminal expectedAtCursor, String content, int cursorOffset) {
		if (cursorOffset < 0) {
			return "";
		}
		int end = 0;
		for (JTL.resource.jtl.mopp.JtlExpectedTerminal expectedElement : expectedElements) {
	...
	protected java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> deriveProposals(java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> expectedElements, String content, JTL.resource.jtl.IJtlTextResource resource, int cursorOffset) {
		java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> resultSet = new java.util.LinkedHashSet<JTL.resource.jtl.ui.JtlCompletionProposal>();
		for (JTL.resource.jtl.mopp.JtlExpectedTerminal expectedElement : expectedElements) {
	...
	protected java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> deriveProposals(final JTL.resource.jtl.mopp.JtlExpectedTerminal expectedTerminal, String content, JTL.resource.jtl.IJtlTextResource resource, int cursorOffset) {
		JTL.resource.jtl.IJtlExpectedElement expectedElement = (JTL.resource.jtl.IJtlExpectedElement) expectedTerminal.getTerminal();
		if (expectedElement instanceof JTL.resource.jtl.mopp.JtlExpectedCsString) {
			JTL.resource.jtl.mopp.JtlExpectedCsString csString = (JTL.resource.jtl.mopp.JtlExpectedCsString) expectedElement;
			return handleKeyword(expectedTerminal, csString, expectedTerminal.getPrefix());
		} else if (expectedElement instanceof JTL.resource.jtl.mopp.JtlExpectedBooleanTerminal) {
			JTL.resource.jtl.mopp.JtlExpectedBooleanTerminal expectedBooleanTerminal = (JTL.resource.jtl.mopp.JtlExpectedBooleanTerminal) expectedElement;
			return handleBooleanTerminal(expectedTerminal, expectedBooleanTerminal, expectedTerminal.getPrefix());
		} else if (expectedElement instanceof JTL.resource.jtl.mopp.JtlExpectedEnumerationTerminal) {
			JTL.resource.jtl.mopp.JtlExpectedEnumerationTerminal expectedEnumerationTerminal = (JTL.resource.jtl.mopp.JtlExpectedEnumerationTerminal) expectedElement;
			return handleEnumerationTerminal(expectedTerminal, expectedEnumerationTerminal, expectedTerminal.getPrefix());
		} else if (expectedElement instanceof JTL.resource.jtl.mopp.JtlExpectedStructuralFeature) {
			final JTL.resource.jtl.mopp.JtlExpectedStructuralFeature expectedFeature = (JTL.resource.jtl.mopp.JtlExpectedStructuralFeature) expectedElement;
			...
			// completed.
			final java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> proposals = new java.util.ArrayList<JTL.resource.jtl.ui.JtlCompletionProposal>();
	...
	protected java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> handleEnumAttribute(JTL.resource.jtl.mopp.JtlExpectedTerminal expectedTerminal, JTL.resource.jtl.mopp.JtlExpectedStructuralFeature expectedFeature, org.eclipse.emf.ecore.EEnum enumType, String prefix, org.eclipse.emf.ecore.EObject container) {
		java.util.Collection<org.eclipse.emf.ecore.EEnumLiteral> enumLiterals = enumType.getELiterals();
		java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> result = new java.util.LinkedHashSet<JTL.resource.jtl.ui.JtlCompletionProposal>();
		for (org.eclipse.emf.ecore.EEnumLiteral literal : enumLiterals) {
			...
			JTL.resource.jtl.IJtlTokenResolverFactory tokenResolverFactory = metaInformation.getTokenResolverFactory();
			JTL.resource.jtl.IJtlTokenResolver tokenResolver = tokenResolverFactory.createTokenResolver(expectedFeature.getTokenName());
			...
			result.add(new JTL.resource.jtl.ui.JtlCompletionProposal(expectedTerminal, resolvedLiteral, prefix, matchesPrefix, expectedFeature.getFeature(), container));
		}
		return result;
	}
	
	protected java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> handleNCReference(JTL.resource.jtl.mopp.JtlExpectedTerminal expectedTerminal, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, String prefix, String tokenName) {
		// proposals for non-containment references are derived by calling the reference
		// resolver switch in fuzzy mode.
		JTL.resource.jtl.IJtlReferenceResolverSwitch resolverSwitch = metaInformation.getReferenceResolverSwitch();
		JTL.resource.jtl.IJtlTokenResolverFactory tokenResolverFactory = metaInformation.getTokenResolverFactory();
		JTL.resource.jtl.IJtlReferenceResolveResult<org.eclipse.emf.ecore.EObject> result = new JTL.resource.jtl.mopp.JtlReferenceResolveResult<org.eclipse.emf.ecore.EObject>(true);
		resolverSwitch.resolveFuzzy(prefix, container, reference, 0, result);
		java.util.Collection<JTL.resource.jtl.IJtlReferenceMapping<org.eclipse.emf.ecore.EObject>> mappings = result.getMappings();
		if (mappings != null) {
			java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> resultSet = new java.util.LinkedHashSet<JTL.resource.jtl.ui.JtlCompletionProposal>();
			for (JTL.resource.jtl.IJtlReferenceMapping<org.eclipse.emf.ecore.EObject> mapping : mappings) {
				org.eclipse.swt.graphics.Image image = null;
				if (mapping instanceof JTL.resource.jtl.mopp.JtlElementMapping<?>) {
					JTL.resource.jtl.mopp.JtlElementMapping<?> elementMapping = (JTL.resource.jtl.mopp.JtlElementMapping<?>) mapping;
					...
					resultSet.add(new JTL.resource.jtl.ui.JtlCompletionProposal(expectedTerminal, identifier, prefix, matchesPrefix, reference, container, image));
				}
			}
			return resultSet;
		}
	...
	protected java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> handleAttribute(JTL.resource.jtl.mopp.JtlExpectedTerminal expectedTerminal, JTL.resource.jtl.mopp.JtlExpectedStructuralFeature expectedFeature, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EAttribute attribute, String prefix) {
		java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> resultSet = new java.util.LinkedHashSet<JTL.resource.jtl.ui.JtlCompletionProposal>();
				...
				if (defaultValue != null) {
					...
					if (tokenName != null) {
						...
						if (tokenResolver != null) {
							...
							resultSet.add(new JTL.resource.jtl.ui.JtlCompletionProposal(expectedTerminal, defaultValueAsString, prefix, matchesPrefix, expectedFeature.getFeature(), container));
						}
					}
				}
			}
	 ...
	 */
	protected java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> handleKeyword(JTL.resource.jtl.mopp.JtlExpectedTerminal expectedTerminal, JTL.resource.jtl.mopp.JtlExpectedCsString csString, String prefix) {
		String proposal = csString.getValue();
		boolean matchesPrefix = matches(proposal, prefix);
		return java.util.Collections.singleton(new JTL.resource.jtl.ui.JtlCompletionProposal(expectedTerminal, proposal, prefix, matchesPrefix, null, null));
	}
	
	/**
	 * Creates a set of (two) completion proposals from the given boolean terminal.
	 */
	protected java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> handleBooleanTerminal(JTL.resource.jtl.mopp.JtlExpectedTerminal expectedTerminal, JTL.resource.jtl.mopp.JtlExpectedBooleanTerminal expectedBooleanTerminal, String prefix) {
		java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> result = new java.util.LinkedHashSet<JTL.resource.jtl.ui.JtlCompletionProposal>(2);
		JTL.resource.jtl.grammar.JtlBooleanTerminal booleanTerminal = expectedBooleanTerminal.getBooleanTerminal();
	 ...
	 */
	protected java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> handleEnumerationTerminal(JTL.resource.jtl.mopp.JtlExpectedTerminal expectedTerminal, JTL.resource.jtl.mopp.JtlExpectedEnumerationTerminal expectedEnumerationTerminal, String prefix) {
		java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> result = new java.util.LinkedHashSet<JTL.resource.jtl.ui.JtlCompletionProposal>(2);
		JTL.resource.jtl.grammar.JtlEnumerationTerminal enumerationTerminal = expectedEnumerationTerminal.getEnumerationTerminal();
	...
	protected java.util.Collection<JTL.resource.jtl.ui.JtlCompletionProposal> handleLiteral(JTL.resource.jtl.mopp.JtlExpectedTerminal expectedTerminal, org.eclipse.emf.ecore.EAttribute attribute, String prefix, String literal) {
		if ("".equals(literal)) {
			return java.util.Collections.emptySet();
		}
		boolean matchesPrefix = matches(literal, prefix);
		return java.util.Collections.singleton(new JTL.resource.jtl.ui.JtlCompletionProposal(expectedTerminal, literal, prefix, matchesPrefix, null, null));
	}
	 ...
	 * element is expected.
	 */
	protected void setPrefixes(java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> expectedElements, String content, int cursorOffset) {
		if (cursorOffset < 0) {
			return;
		}
		for (JTL.resource.jtl.mopp.JtlExpectedTerminal expectedElement : expectedElements) {
	...
	public JTL.resource.jtl.mopp.JtlExpectedTerminal[] getElementsExpectedAt(JTL.resource.jtl.mopp.JtlExpectedTerminal[] allExpectedElements, int cursorOffset) {
		java.util.List<JTL.resource.jtl.mopp.JtlExpectedTerminal> expectedAtCursor = new java.util.ArrayList<JTL.resource.jtl.mopp.JtlExpectedTerminal>();
		for (int i = 0; i < allExpectedElements.length; i++) {
			JTL.resource.jtl.mopp.JtlExpectedTerminal expectedElement = allExpectedElements[i];
		...
		return expectedAtCursor.toArray(new JTL.resource.jtl.mopp.JtlExpectedTerminal[expectedAtCursor.size()]);
	}
	 ...
	 */
	protected int getEnd(JTL.resource.jtl.mopp.JtlExpectedTerminal[] allExpectedElements, int indexInList) {
		JTL.resource.jtl.mopp.JtlExpectedTerminal elementAtIndex = allExpectedElements[indexInList];
		...
		for (int i = indexInList + 1; i < allExpectedElements.length; i++) {
			JTL.resource.jtl.mopp.JtlExpectedTerminal elementAtI = allExpectedElements[i];
	...
	protected boolean matches(String proposal, String prefix) {
		...
		return (proposal.toLowerCase().startsWith(prefix.toLowerCase()) || JTL.resource.jtl.util.JtlStringUtil.matchCamelCase(prefix, proposal) != null) && !proposal.equals(prefix);
	}
	...
	protected org.eclipse.emf.ecore.EObject findCorrectContainer(JTL.resource.jtl.mopp.JtlExpectedTerminal expectedTerminal) {
		...
		JTL.resource.jtl.grammar.JtlContainmentTrace containmentTrace = expectedTerminal.getContainmentTrace();
		org.eclipse.emf.ecore.EClass startClass = containmentTrace.getStartClass();
		JTL.resource.jtl.mopp.JtlContainedFeature currentLink = null;
		JTL.resource.jtl.mopp.JtlContainedFeature previousLink = null;
		JTL.resource.jtl.mopp.JtlContainedFeature[] containedFeatures = containmentTrace.getPath();
			...
			if (hookableParent != null) {
				...
				if (parent != null) {
					if (previousParent == null) {
						...
						JTL.resource.jtl.mopp.JtlContainedFeature link = previousLink;
						JTL.resource.jtl.util.JtlEObjectUtil.setFeature(parent, link.getFeature(), previousParent, false);
					}
				}
			}
		}
		...
		if (parent != null && hookableParent != null) {
			expectedTerminal.setAttachmentCode(new Runnable() {
				...
				public void run() {
					JTL.resource.jtl.util.JtlEObjectUtil.setFeature(finalHookableParent, finalFeature, finalParent, false);
				}
			});
		}
		return correctContainer;
	 ...
	 */
	protected org.eclipse.emf.ecore.EObject findHookParent(org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EClass startClass, JTL.resource.jtl.mopp.JtlContainedFeature currentLink, org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EClass containerClass = currentLink.getContainerClass();
