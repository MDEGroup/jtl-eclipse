...
public class JtlBracketSet {
	...
	private java.util.ArrayList<JTL.resource.jtl.IJtlBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements JTL.resource.jtl.IJtlBracketPair {
	 ...
	 */
	private class ClosingListener implements JTL.resource.jtl.ui.IJtlBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				JTL.resource.jtl.IJtlBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public JtlBracketSet(JTL.resource.jtl.ui.JtlEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new JTL.resource.jtl.mopp.JtlMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<JTL.resource.jtl.IJtlBracketPair>();
	 ...
	 */
	public JTL.resource.jtl.IJtlBracketPair getBracketPair(String opening, String closing) {
		for (JTL.resource.jtl.IJtlBracketPair bracketPair : bracketPairs) {
	...
	public JTL.resource.jtl.IJtlBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(JTL.resource.jtl.IJtlBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + JTL.resource.jtl.ui.JtlPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public JTL.resource.jtl.IJtlBracketPair remove(String opening, String closing) {
		for (JTL.resource.jtl.IJtlBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<JTL.resource.jtl.IJtlBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, JTL.resource.jtl.ui.JtlPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, JTL.resource.jtl.ui.JtlPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
